import { ISignPosition, InitCanvasImageData, UpdateSignList, ICanvasPosition } from "../globalInterFace";
import { v4 } from "uuid";
...
// 返回canvas相对于视口的位置
export const getTouchPosition = (event: any, scale: number) => {
...
// 返回当前的偏移值
export const getCanvasStandPosition = (event: any) => {
    // const event = getEvent(e);
    const target = event.target;
    const rect = target.getBoundingClientRect();
    const x = event.pageX - rect.left;
